...
#
# Credit for how to fix the Berkley DB goes to: https://fsanmartin.co/running-a-bitcoin-node-on-ubuntu-19-10/
#
...
# 1. Install the pre-requisites via apt
# 2. Clone the repo
# 3. Compile, build and install the Berkley DB
# 4. Compile, build and install Solidus
# 5. Add nodes and start the solidus daemon to begin building the block chain
# 
...
#
sudo apt-get install git curl -y #You need git to clone the repo
sleep 2
...
cd ~/solidus
echo 'We need to fix and update the installation with some patches'
# Insert lines to fetch the latest config.guess and config.sub files and make them executable
# These are required to build the DB on a Raspberry PI or any newer architecture post 2009
sed '/^cd "${BDB_PREFIX*/a curl https://git.savannah.gnu.org/cgit/config.git/plain/config.guess > ./dist/config.guess\ncurl https://git.savannah.gnu.org/cgit/config.git/plain/config.sub > ./dist/config.sub\nchmod +x ./dist/config.guess ./dist/config.sub' ./contrib/install_db4.sh > ./contrib/install.new
# fix the bug on BerkeleyDB which impacts builds on > ubuntu 19.04
sed -i 's/__atomic_compare_exchange/__atomic_compare_exchange_db/g' ~/solidus/db4/db-4.8.30.NC/dbinc/atomic.h
#
# Start the build of the DB 
chmod +x ./contrib/install.new
sudo ./contrib/install.new `pwd`
#
...
cd ~/solidus
currentpath=$(pwd)
export BDB_PREFIX=$currentpath/db4
echo $BDB_PREFIX
...
# > solidus-qt &
#
# Note: you cannot run solidusd and solidus-qt at the same time, it's one or the other,so if you have the daemon running, you need to stop it first.
